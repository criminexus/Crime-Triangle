...
to-report crime_process [#list_involved]
  ...
  repeat length _list_pedestrians_involved [
    ifelse (random 99 < threshold_pedestrians) [set outcome "killed"][set outcome "safe"]
  ...
  repeat length _list_offenders_involved [
      ifelse (length #list_involved = 3) [
        ifelse (random  99 < threshold_offenders) [set outcome "killed"][set outcome "safe"]][
        set outcome "safe"]
  ...
  if (length #list_involved = 3) [
    repeat length _list_guardians_involved [
      ifelse (random  99 < threshold_guardians) [set outcome "killed"][set outcome "safe"]
...
Offender agents can have three activity statuses: 
-not_considering_crime; 
-scanning; 
-committing_crime.

When a crime event happens, the crime agent that is generated can have one of the following statuses: 
-crime_started; 
-crime_in_progress; 
-crime_finishing; 
-crime ended. 
...
Use the sliders to change the conditional probabilities that the agents are killed during the crime event:
-threshold pedestrians 
-threshold guardians
-threshold offenders
